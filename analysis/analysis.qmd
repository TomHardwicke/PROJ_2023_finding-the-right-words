...
rankSimplePlot <- function(d){
  d %>%
    ...
    geom_pointrange(aes(ymin = lwr.ci, ymax = upr.ci), size = 1.1, linewidth = 1.3, shape = 21, fill = "white", colour = "black") +
    #geom_text(aes(label = percentCI), nudge_y = 15) +
    scale_colour_manual(values = c('Match' = okabe[1], 'No match' = okabe[5]), guide = 'none') +
    ylim(0,100) +
    xlab('vocabularly') +
...
rank_simple_plot <- bind_rows(
  rank_summary_elife_import %>%
  rankSimple() %>%
  mutate(vocab = 'eLife', dimension = 'Importance dimension'),
  rank_summary_alt_import %>%
  rankSimple() %>%
  mutate(vocab = 'Alternative', dimension = 'Importance dimension'),
  rank_summary_elife_support %>%
  rankSimple() %>%
  mutate(vocab = 'eLife', dimension = 'Support dimension'),
  rank_summary_alt_support %>%
  rankSimple() %>%
  mutate(vocab = 'Alternative', dimension = 'Support dimension'),
) %>% 
...
kd_plot <- bind_rows(
  ...
  scale_fill_manual(values = c("TRUE" = okabe[1], "FALSE" = "grey")) +
